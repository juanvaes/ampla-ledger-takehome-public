...
def balances(ctx: Dict, end_date: str = None) -> None:
    ...
    with sqlite3.connect(ctx.obj["DB_PATH"]) as connection:
        ...
        ) = advance_calculator.get_advance_statistics(events, end_date)
    ...
    for advance_id, advance_data in advance_calculator.advances.items():
        click.echo(
            "{0:10}{1:>11}{2:>17.2f}{3:>20.2f}".format(
                advance_id,
                convert_date_to_string(advance_data["Date"]),
                advance_data["Initial Amt"],
                advance_data["Current Balance"],
            )
        )
